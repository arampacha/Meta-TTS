...
def prepare_raw_data(datafiles):
    ...
    for datafile in datafiles:
        with open(datafile) as f:
            items = [DataItem(*row.split('\t')) for row in f.readlines()]
        split = Path(datafile).stem
        dataset[split] = items
        for item in items:
            shutil.copy(item.filename, f'raw_data/custom/{item.speaker}/{item.filename}')
            with open(f"raw_data/custom/{item.speaker}/{Path(item.filename).with_suffix('lab')}") as f:
                f.write(item.transcript)
    try:
        run_mfa("raw_data/custom")
    except Exception as e:
        print(e)

    for split, items in dataset.items():
        shutil.move(f'raw_data/custom/{item.speaker}/{item.filename}', f'raw_data/custom/{split}/{item.speaker}/{item.filename}')
        shutil.move(
            f"raw_data/custom/{item.speaker}/{Path(item.filename).with_suffix('lab')}",
            f"raw_data/custom/{split}/{item.speaker}/{Path(item.filename).with_suffix('lab')}"
        )
    
def preprocess():
    subprocess.run("python preprocess.py config/preprocess/ucg.yaml")
